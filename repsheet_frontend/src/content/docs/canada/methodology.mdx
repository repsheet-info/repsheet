...
4. **MP summarization stage 2**: Finally, we take the MP summaries from each batch and ask the AI to merge them into a final combined summary of the MP (3rd prompt).
...
Model: <a href="https://www.anthropic.com/claude/haiku" target="_blank"  rel="noopener noreferrer">Claude Haiku</a>
...
* The voting statistics of all other parties (yeas/nays/abstains, party name not included)
...
We use Claude Haiku as we found it provided high quality summaries and was fast and reasonably priced, as we need to run a large number of these summaries.
...
The final step is to combine the separate "batched" MP summaries of the MP's voting records into one unified summary of the MP.
